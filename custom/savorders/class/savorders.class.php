...
class savorders
{
    ...
    public function initsavordersModule($lastversion = '')
    {
        ...
        // SAV Quantity Received on line
        $extrafields->addExtraField('line_sav_qty_recue', $langs->trans('SAV Quantity Received On Line'), "integer", $position++, '', 'commandedet', 0, 0, '', '', 0, '', $list);
        // SAV Quantity Delivered on line
        $extrafields->addExtraField('line_sav_qty_livree', $langs->trans('SAV Quantity Delivered On Line'), "integer", $position++, '', 'commandedet', 0, 0, '', '', 0, '', $list);

        // --------------------------------------------------------- Order Lines (Supplier)
        // SAV Quantity Delivered to supplier on line (equivalent to recue for customer)
        $extrafields->addExtraField('line_sav_qty_envoyee_fourn', $langs->trans('SAV Quantity Sent To Supplier On Line'), "integer", $position++, '', 'commande_fournisseurdet', 0, 0, '', '', 0, '', $list);
        // SAV Quantity Received from supplier on line (equivalent to livree for customer)
        $extrafields->addExtraField('line_sav_qty_recue_fourn', $langs->trans('SAV Quantity Received From Supplier On Line'), "integer", $position++, '', 'commande_fournisseurdet', 0, 0, '', '', 0, '', $list);
