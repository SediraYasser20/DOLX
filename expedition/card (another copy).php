    ...
    if (!$error) {
		...
		if ($result < 0) {
			...
			if ((!getDolGlobalString('MAIN_DISABLE_PDF_AUTOUPDATE') && isset($result_pdf) && $result_pdf >= 0) || getDolGlobalString('MAIN_DISABLE_PDF_AUTOUPDATE')) {
				...
				require_once DOL_DOCUMENT_ROOT.'/core/lib/files.lib.php'; // For dol_most_recent_file

				// 1. Determine Recipient (To)
				// Default to thirdparty email. If not available, try to find a contact.
				$sendto_auto = '';
				if (!empty($object->thirdparty->email)) {
					$sendto_auto = $object->thirdparty->email;
				} else {
					// Try to get the first external contact's email
					$contacts = $object->thirdparty->liste_contact(-1, 'external', 1); // Get 1 external contact
				...
				if (!empty($sendto_auto)) {
					// 2. Determine Sender (From) - Using company email as a robust default
					$from_auto_name = dol_string_nospecial($conf->global->MAIN_INFO_SOCIETE_NOM, ' ', array(","));
					$from_auto_email = getDolGlobalString('MAIN_INFO_SOCIETE_MAIL');
					if (empty($from_auto_email)) { // Fallback to user email if company email is not set
					...
					// 3. Determine Subject
					// $outputlangs should be set from the PDF generation block above
					$subject_auto = $outputlangs->transnoentities("SendShippingRef", $object->ref, $object->thirdparty->name);
					if (!empty($object->ref_customer)) {
						$subject_auto = $outputlangs->transnoentities("SendShippingRefWithCustRef", $object->ref, $object->ref_customer, $object->thirdparty->name);
					}
					
					// 4. Determine Message Body (using a default message or a specific email template if defined)
					// For now, a simple default message. Users might want to customize this via a template later.
					// Substitution array
					...
					$default_message_body_key = "EmailBodyShippingValidated"; // A new lang key for the default body
					$langs->loadLangs(array("other")); // Ensure 'other' is loaded if it contains the new key
					$message_auto_raw = $outputlangs->transnoentities($default_message_body_key);
					// If the key is not found, provide a fallback message
					if ($message_auto_raw == $default_message_body_key) {
                    ...
                    // Convert to HTML if needed, assuming $message_auto is plain text for now, or make it HTML
                    $message_auto = nl2br($message_auto);


					// 5. Determine Attachments
					$filepath_auto = array();
					$filename_auto = array();
					$mimetype_auto = array();

					$objectref_sanitized = dol_sanitizeFileName($object->ref);
                    ...
                    if (!empty($object->last_main_doc) && is_readable(DOL_DATA_ROOT.'/'.$object->last_main_doc) && is_file(DOL_DATA_ROOT.'/'.$object->last_main_doc)) {
                        ...
                        // Fallback to dol_most_recent_file if last_main_doc is not set or invalid
					...
					if (!empty($file_to_attach_info['fullname']) && is_readable($file_to_attach_info['fullname'])) {
						...
						setEventMessages($langs->trans("ErrorFailedToFindAttachmentForAutoEmail", $object->ref), null, 'warnings');
					}

					// 6. BCC, Delivery Receipt etc.
					$sendtocc_auto = '';
					$sendtobcc_auto = getDolGlobalString('MAIN_MAIL_AUTOCOPY_SHIPMENT_TO'); // Use the existing $autocopy variable's target
					$deliveryreceipt_auto = getDolGlobalInt('MAIN_MAIL_SEND_DELIVERY_RECEIPT'); // Use global setting
					...
					if ($mailfile->sendfile()) {
						...
						// Call trigger
						$triggersendname_auto = 'SHIPPING_SENTBYMAIL_AUTO'; // Could be same or different
						$object->sendtoid = array(); // Assuming no specific contact id for auto-send, or fetch primary contact id
						// Try to get thirdparty's main contact ID if possible for logging
						...
						$object->actionmsg = $message_auto; // Full message
						...
						$object->trackid = 'shi'.$object->id.'_auto_'.time(); // Unique trackid for this auto send
						...
						setEventMessages(null, array($errorMessage), 'errors'); // Pass as array for errors
					}
				} else {
					setEventMessages($langs->trans("ErrorNoRecipientEmailForAutoSend", $object->thirdparty->name), null, 'warnings');
				}
			}
		}
