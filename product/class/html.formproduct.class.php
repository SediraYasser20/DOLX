...
class FormProduct
{
	 ...
	 *  @return string|int
	 */
	public function selectProductNature($name = 'finished', $selected = '', $mode = 0, $showempty = 1, $required = false)
	{
		global $langs, $db;
		...
		if ($result < 0) {
			...
			$return .= '<select class="flat" name="'.$name.'" id="'.$name.'"'.($required ? ' required' : '').'>';
